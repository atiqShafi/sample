...
namespace Sample.Web.Mvc.Bundles
{
    ...
    public class TemplateCompiler
    {
        ...
        private void Compile<T>(TextWriter writer, T[] templateFiles) where T : VirtualFile
        {
            ...
            var appName = HttpContext.Current.Request.ApplicationPath.TrimStart('/') + "/";
            foreach (var file in templateFiles)
            {
                var templateName = GetTemplateName(file.VirtualPath);
                if (templateName.StartsWith(appName))
                {
                    templateName = templateName.ReplaceFirst(appName, "");
                }
                string template;
